...
import okble.central.client.DebugBluetoothGattCallback;
...
public class DeviceViewModel extends AndroidViewModel {
    ...
    private MutableLiveData<List<GattData>> mGattData = new MutableLiveData<>();
    public LiveData<List<GattData>> getGattData(){
        return mGattData;
    }

    private MutableLiveData<DescriptorData> mDescriptorData = new MutableLiveData<>();
    public LiveData<DescriptorData> getDescriptorData(){
        return mDescriptorData;
    }

    private MutableLiveData<CharacteristicData> mCharacteristicData = new MutableLiveData<>();
    public LiveData<CharacteristicData> getCharacteristicData(){
        return mCharacteristicData;
    }
    ...
    public void readCharacteristic(final BluetoothGattCharacteristic characteristic){
        ...
        final UUID sUuid = characteristic.getService().getUuid();
        final ReadCharacteristicRequest req = new ReadCharacteristicRequest.Builder()
        ...
        task.addOnCompleteListener(new OnCompleteListener<byte[]>(){
            ...
            public void onComplete(OkBleTask<byte[]> task) {
                if(task.isSuccess()){
                    mCharacteristicData.setValue(CharacteristicData.from(characteristic, task.getResult()));
                }
            }
        });
        task.enqueue();
    ...
    public void readDescriptor(BluetoothGattDescriptor descriptor){
        ...
        task.addOnCompleteListener(new OnCompleteListener<byte[]>(){
            ...
            public void onComplete(OkBleTask<byte[]> task) {
                if(task.isSuccess()){
                    mDescriptorData.setValue(DescriptorData.from(descriptor, task.getResult()));
                }
            }
        });
        task.enqueue();
        ...
        client.addOnConnectionStateChangedListener(new OnConnectionStateChangedListener(){
            ...
            public void onConnectionStateChanged(OkBleClient client, ConnectionState newState, ConnectionState lastState) {
                ...
                if(newState == ConnectionState.Connected){
                    final List<BluetoothGattService> services = client.getServices();
                    List<GattData> list = GattData.from(services);
                    mGattData.setValue(list);
                }else if(newState == ConnectionState.Disconnected){
//                    mGattData.setValue(null);
                }else{
//                    mGattData.setValue(null);
                }
            }
        ...
        client.setDebugBluetoothGattCallback(new DebugBluetoothGattCallback(){
            @Override
            public void onConnectionStateChange(OkBleClient client, int status, int newState) {
                final long now = System.currentTimeMillis();
                final String msg = String.format("onConnectionStateChange. status:%d, newState:%d", status, newState);
                final LogInfo info = new LogInfo(now, msg);
                mLogInfo.postValue(info);
            }

            @Override
            public void onServicesDiscovered(OkBleClient client, int status) {
                final long now = System.currentTimeMillis();
                final String msg = String.format("onServicesDiscovered. status:%d", status);
                final LogInfo info = new LogInfo(now, msg);
                mLogInfo.postValue(info);
            }

            @Override
            public void onCharacteristicChanged(OkBleClient client, CharacteristicData data) {
                final long now = System.currentTimeMillis();
                final String msg = String.format("onCharacteristicChanged. %s", data);
                final LogInfo info = new LogInfo(now, msg);
                mLogInfo.postValue(info);
            }

            @Override
            public void onCharacteristicWrite(OkBleClient client, CharacteristicData data, int status) {
                final long now = System.currentTimeMillis();
                final String msg = String.format("onCharacteristicWrite. status:%s, %s",status, data);
                final LogInfo info = new LogInfo(now, msg);
                mLogInfo.postValue(info);
            }

            @Override
            public void onCharacteristicRead(OkBleClient client, CharacteristicData data, int status) {
                final long now = System.currentTimeMillis();
                final String msg = String.format("onCharacteristicRead. status:%d, %s",status, data);
                final LogInfo info = new LogInfo(now, msg);
                mLogInfo.postValue(info);
            }

            @Override
            public void onDescriptorRead(OkBleClient client, DescriptorData data, int status) {
                final long now = System.currentTimeMillis();
                final String msg = String.format("onDescriptorRead. status:%d, %s",status, data);
                final LogInfo info = new LogInfo(now, msg);
                mLogInfo.postValue(info);
            }

            @Override
            public void onDescriptorWrite(OkBleClient client, DescriptorData data, int status) {
                final long now = System.currentTimeMillis();
                final String msg = String.format("onDescriptorWrite. status:%d, %s",status, data);
                final LogInfo info = new LogInfo(now, msg);
                mLogInfo.postValue(info);
            }

            @Override
            public void onPhyUpdate(OkBleClient client, int txPhy, int rxPhy, int status) {
                final long now = System.currentTimeMillis();
                final String msg = String.format("onPhyUpdate. status:%d , txPhy:%d , rxPhy:%d", status, txPhy, rxPhy);
                final LogInfo info = new LogInfo(now, msg);
                mLogInfo.postValue(info);
            }

            @Override
            public void onPhyRead(OkBleClient client, int txPhy, int rxPhy, int status) {
                final long now = System.currentTimeMillis();
                final String msg = String.format("onPhyRead. status:%d , txPhy:%d , rxPhy:%d", status, txPhy, rxPhy);
                final LogInfo info = new LogInfo(now, msg);
                mLogInfo.postValue(info);
            }

            @Override
            public void onMtuChanged(OkBleClient client, int mtu, int status) {
                final long now = System.currentTimeMillis();
                final String msg = String.format("onMtuChanged. status:%d , mtu:%d", status, mtu);
                final LogInfo info = new LogInfo(now, msg);
                mLogInfo.postValue(info);
            }

            @Override
            public void onReadRemoteRssi(OkBleClient client, int rssi, int status) {
                final long now = System.currentTimeMillis();
                final String msg = String.format("onReadRemoteRssi. status:%d , rssi:%d", status, rssi);
                final LogInfo info = new LogInfo(now, msg);
                mLogInfo.postValue(info);
            }

            @Override
            public void onConnectionUpdated(OkBleClient client, int interval, int latency, int timeout, int status) {
                final long now = System.currentTimeMillis();
                final String msg = String.format("onConnectionUpdated. status:%d, interval:%s, latency:%s, timeout:%s",
                        status, interval, latency, status);
                final LogInfo info = new LogInfo(now, msg);
                mLogInfo.postValue(info);
            }
        });
    ...
    private MutableLiveData<LogInfo> mLogInfo = new MutableLiveData<>();
    public LiveData<LogInfo> getLogInfo(){
        return mLogInfo;
    }
