...
final class DisableNotificationExecutor extends RequestExecutor {

    @Override
    public void executeRequest(ClientOperation client, OkBleTask0 task, Request<?> request0) {
        final DisableNotificationRequest request = (DisableNotificationRequest)request0;
        final UUID service = request.service();
        final UUID characteristic = request.characteristic();
        try{
            final long now = System.currentTimeMillis();
            final boolean success = client.doDisableNotification(service, characteristic);
